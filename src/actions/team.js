...
export function connectToChannel(socket, teamId) {
  return dispatch => {
    ...
    channel.on('reservation_deleted', message => {
      ...
      const notification = `${reservation.environment.name}:${
...
export function updateEnvironment(environment) {
  return dispatch =>
    api
      .patch(
        `/teams/${environment.team_id}/environments/${environment.id}`,
        environment
      )
      .then(response => {
...
export function updateApplication(application) {
  return dispatch =>
    api
      .patch(
        `/teams/${application.team_id}/applications/${application.id}`,
        application
      )
      .then(response => {
